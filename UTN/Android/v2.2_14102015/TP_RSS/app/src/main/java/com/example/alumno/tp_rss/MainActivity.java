...
public class MainActivity extends AppCompatActivity implements Handler.Callback {
    ...
    public void clickNoticia(int posicionNoticiaClickeada ) {
        ...
        intent.putExtra("urlNoticia", notice.getLinkNoticia());
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 1) {
            if(resultCode == RESULT_OK){
                Log.d("volvio", "volvio OK");
                Toast.makeText( this.getBaseContext(), "Retornando del intent OK", Toast.LENGTH_SHORT).show();
            }
            if (resultCode == RESULT_CANCELED) {
                //Write your code if there's no result
                Log.d("volvio...", "volvio sin resultado");
                Toast.makeText( this.getBaseContext(), "Retornando del intent con ERROR", Toast.LENGTH_SHORT).show();
            }
        }
    }//onActivityResult
